 ...
 */
public class CollapseViewGroup3 extends RelativeLayout implements View.OnTouchListener, View.OnClickListener {
    ...
    private float discreteNumber = 1.0f;
    ...
    public CollapseViewGroup3(Context context) {
        super(context);
        init(context);
    }

    public CollapseViewGroup3(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public CollapseViewGroup3(Context context, AttributeSet attrs, int defStyleAttr) {
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (scrollY < 0) {
                        ...
                        if (isOpen) {
                            if (mMoveHeight == 0) {
                                float a = 0.0f;
                                a = (float) Math.abs(scrollY) / mItemHeight;
                                float discreteA = a*discreteNumber;
                                if(discreteA>1.0f){
                                    discreteA=1.0f;
                                }
                                int top = ((int) (-discreteA * (mItemHeight * 3)) + mCalendarHeight);
                              //  if (top >ItemHeight) {
                                Log.d("BBBBBBgetgetScrollY","=============");
                                Log.d("BBBBBBgetTop",mLinBottom.getTop()+"");
                                Log.d("BBBBBBgetY",mLinBottom.getY()+"");
                                Log.d("BBBBBBgetgetScrollY",mLinBottom.getScrollY()+"");
                                Log.d("BBBBBBgetgetTranslationY",mLinBottom.getTranslationY()+"");
                                Log.d("BBBBBBmItemHeight",mItemHeight+"");
                                if (mLinBottom.getTop() >= mItemHeight) {
                                    mLinBottom.setTop(top);
                                }
                            } else if (Math.abs(scrollY) <= mMoveHeight) {
                                float a = 0.0f;
                                a = (float) Math.abs(scrollY) / mMoveHeight;

                                float discreteA = a*discreteNumber;
                                if(discreteA>1.0f){
                                    discreteA=1.0f;
                                }
                                mLinCalendar.setTop(scrollY);
                                int top = ((int) (-discreteA * (mItemHeight * 3)) + mCalendarHeight);
                                mLinBottom.setTop(top);
                        ...
                        if (!isOpen) {
                            if (mMoveHeight == 0) {
                                float a = 0.0f;
                                a = (float) scrollY / mItemHeight;
                                int top = ((int) (a * (mItemHeight * 3)) + mItemHeight);
                                if (top <= mCalendarHeight) {
                                    mLinBottom.setTop(top);
                                }
                            } else if (Math.abs(scrollY) <= mMoveHeight) {
                                float a = 0.0f;
                                a = (float)scrollY / mMoveHeight;
                                mLinCalendar.setTop(-mMoveHeight+scrollY);
                                int top = ((int) (a * (mItemHeight * 3)) + mItemHeight);
                                mLinBottom.setTop(top);
                            }
                        }
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if (downY > upY) {
                        ...
                        if (mMoveHeight == 0) {
                            Log.d("AAAAAAAbottomMoveAlready",bottomMoveAlready+"");
                            Log.d("AAAAAAAhalfBottombottomMoveDelay",bottomMoveDelay+"");
                            Log.d("AAAAAAAhalfBottomMoveDistance",halfBottomMoveDistance+"");

                            Log.d("AAAAAAAgetTop",mLinBottom.getTop()+"");
                            Log.d("AAAAAAAgetY",mLinBottom.getY()+"");
                            Log.d("AAAAAAAmCalendarHeight",mCalendarHeight+"");
                            //第一行
                            if((int)mLinBottom.getY() ==mCalendarHeight){
                                //已经在最下面
                                //用getY而不是top
                                Log.d("AAAAAAAhalfBottomMoveDistance","false");
                                return false ;
                            }
                            Log.d("AAAAAAAhalfBottomMoveDistance","true");
                            if (bottomMoveAlready >= halfBottomMoveDistance) {

                                AniUtils.up(mLinCalendar, 0, mLinBottom, -bottomMoveDelay);
                              //  mLinBottom.setTop(mCalendarHeight);
                                isOpen = true;
                            }else{
                                AniUtils.up(mLinCalendar, 0, mLinBottom, bottomMoveAlready);
                                isOpen = false;
