...
public class DrawbleUtils {
    // GradientDrawable
    public static GradientDrawable setGradientDrawable(String color,
                                                       int rLeftTop, int rRightTop, int rRightBottom, int rLeftBottom) {
        ...
        gd.setShape(GradientDrawable.RECTANGLE);// 矩形
        float[] r = { ScreenUtils.convertDipToPx(rLeftTop),
                ScreenUtils.convertDipToPx(rLeftTop),
                ScreenUtils.convertDipToPx(rRightTop),
                ScreenUtils.convertDipToPx(rRightTop),
                ScreenUtils.convertDipToPx(rRightBottom),
                ScreenUtils.convertDipToPx(rRightBottom),
                ScreenUtils.convertDipToPx(rLeftBottom),
                ScreenUtils.convertDipToPx(rLeftBottom) };
    ...
    public static StateListDrawable setStateDrawable(String colorNormal,
                                                     String colorPressed, String colorFocused, int r) {
        StateListDrawable sd = new StateListDrawable();
        Drawable normal = colorNormal == null ? null : setGradientDrawable(
                colorNormal, r, r, r, r);
        Drawable pressed = colorPressed == null ? null : setGradientDrawable(
                colorPressed, r, r, r, r);
        Drawable focus = colorFocused == null ? null : setGradientDrawable(
                colorFocused, r, r, r, r);
    ...
    public static StateListDrawable setStateDrawable(Context context, int idNormal,
    ...
    public static StateListDrawable setStateDrawableRoundedCorner(int idNormal,
                                                     int idPressed, int idFocused,int r) {
        StateListDrawable sd = new StateListDrawable();
        Drawable normal = idNormal == 0 ? null : getRoundedCornerBitmapDrawable(idNormal,r);
        Drawable pressed = idPressed == 0 ? null :  getRoundedCornerBitmapDrawable(idPressed, r);
        Drawable focus = idFocused == 0 ? null :  getRoundedCornerBitmapDrawable(idFocused, r);
        // 注意该处的顺序，只要有一个状态与之相配，背景就会被换掉
        // 所以不要把大范围放在前面了，如果sd.addState(new[]{},normal)放在第一个的话，就没有什么效果了
        sd.addState(new int[] { android.R.attr.state_enabled,
                android.R.attr.state_focused }, focus);
     ...
     * @return
     */
    public static BitmapDrawable getRoundedCornerBitmapDrawable(int idDrawable, int roundPx) {

        Drawable drawable = MyApplication.getContext().getResources().getDrawable(idDrawable);
        ...
        canvas.drawARGB(0, 0, 0, 0);
        paint.setColor(color);
        canvas.drawRoundRect(rectF, ScreenUtils.convertDipToPx(roundPx), ScreenUtils.convertDipToPx(roundPx), paint);
        paint.setXfermode(new PorterDuffXfermode(Mode.SRC_IN));
        canvas.drawBitmap(bitmap, rect, rect, paint);
